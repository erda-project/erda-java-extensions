...
public class AbstractRedisClientInterceptor implements InstanceMethodsAroundInterceptor {
    ...
    public void beforeMethod(IMethodInterceptContext context, MethodInterceptResult result) throws Throwable {
        DynamicFieldEnhancedInstance clientOptions = (DynamicFieldEnhancedInstance) context.getArguments()[0];
        if (clientOptions == null) {
            return;
        }
        AbstractRedisClient client = (AbstractRedisClient) context.getInstance();
        if (client.getOptions() == null || ((DynamicFieldEnhancedInstance) client.getOptions()).getDynamicField() == null) {
            return;
        }
        clientOptions.setDynamicField(((DynamicFieldEnhancedInstance) client.getOptions()).getDynamicField());
    }

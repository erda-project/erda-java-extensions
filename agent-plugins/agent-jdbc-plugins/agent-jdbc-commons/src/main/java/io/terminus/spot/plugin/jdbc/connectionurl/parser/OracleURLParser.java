...
public class OracleURLParser extends AbstractURLParser {
    ...
    private ConnectionInfo commonsURLParse() {
        ...
        if (hostSegment.length == 1) {
            return new ConnectionInfo(Constants.Tags.COMPONENT_ORACLE, DB_TYPE, host, DEFAULT_PORT, databaseName);
        } else {
            return new ConnectionInfo(Constants.Tags.COMPONENT_ORACLE, DB_TYPE, hostSegment[0], Integer.valueOf(hostSegment[1]), databaseName);
        }
    }
    ...
    private ConnectionInfo tnsNameURLParse() {
        ...
        String databaseName = fetchDatabaseNameFromURL();
        return new ConnectionInfo(Constants.Tags.COMPONENT_ORACLE, DB_TYPE, host, databaseName);
    }
    ...
    private String parseDatabaseHostsFromURL() {
        ...
        return Strings.join(',', hosts.toArray(new String[0]));
    }

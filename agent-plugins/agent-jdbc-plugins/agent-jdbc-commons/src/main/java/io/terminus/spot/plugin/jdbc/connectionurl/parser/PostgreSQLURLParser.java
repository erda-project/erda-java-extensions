...
public class PostgreSQLURLParser extends AbstractURLParser {
    ...
    public ConnectionInfo parse() {
        ...
        if (hostSegment.length > 1) {
            ...
            return new ConnectionInfo(Constants.Tags.COMPONENT_POSTGRESQL, DB_TYPE, sb.toString(), fetchDatabaseNameFromURL());
        } else {
            String[] hostAndPort = hostSegment[0].split(":");
            if (hostAndPort.length != 1) {
                return new ConnectionInfo(Constants.Tags.COMPONENT_POSTGRESQL, DB_TYPE, hostAndPort[0], Integer.valueOf(hostAndPort[1]), fetchDatabaseNameFromURL());
            } else {
                return new ConnectionInfo(Constants.Tags.COMPONENT_POSTGRESQL, DB_TYPE, hostAndPort[0], DEFAULT_PORT, fetchDatabaseNameFromURL());
            }
        }
    }

...
package io.terminus.spot.plugin.spring.concurrent.match;
...
/**
 * {@link ListenableFutureCallbackMatch} match the class that inherited <code>org.springframework.util.concurrent
 * .ListenableFutureCallback</code>.
 *
 * @author zhangxin
 */
public class ListenableFutureCallbackMatch implements IndirectMatch {

    private static final String LISTENABLE_FUTURE_CALLBACK_CLASS_NAME
    ...
    public ElementMatcher.Junction buildJunction() {
        return not(nameStartsWith(EitherInterfaceMatch.SPRING_PACKAGE_PREFIX))
    ...
    private boolean matchExactClass(TypeDescription.Generic clazz) {
        ...
        if (!isMatch) {
            ...
            if (superClazz != null && !EitherInterfaceMatch.OBJECT_CLASS_NAME.equals(clazz.getTypeName())) {
                isMatch = matchExactClass(superClazz);
            }
        }
